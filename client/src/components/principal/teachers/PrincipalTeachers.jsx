...
const PrincipalTeachers = () => {
  ...
  const principalId = principalData?.id
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (errorMsgs.some(msg => msg === respons.msg || respons.message)) {
  ...
  const handleUpdation = async (formDatat) => {
    ...
    try {
      ...
      if (result == "Access Denied" || result.message === "jwt malformed") {
        ...
        setExistingTeachers((existingTeachers) => {
          return existingTeachers?.map((teacher) => {
            return teacher?._id === result.teacher?._id
              ? result.teacher
              : teacher;
          });
        });
  ...
  const handleClick = async(userId, event) => {
    event.stopPropagation();
    try {
      setLoading(true);
      const response = await CreatePrincipalConversation(
        token,
        principalId,
        userId
      );
      if (errorMsgs.some((msg) => msg === response.msg || response.message)) {
        ...
        response.msg === "ConvCreated" ||
        response.msg === "ConvFinded"
      ) {
        navigate("/principal/chat");
      }
    } catch (err) {
      console.log(err);
    }
      ...
      {addTeacher && (
        <div className="fixed top-0 left-0 right-0 bottom-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
          <PrincipalAddTeacher
            ...
            loading={loading}
            handleAddTeacherSubmit={handleAddTeacherSubmit}
          />
        </div>
      )}
      {loading && (
      ...
      <div className="p-4 sm:ml-64 h-full align-middle">
        <div className="flex justify-between mx-5">
          ...
          <div className="bg-fuchsia-300 rounded-2xl mb-5 hover:bg-fuchsia-400">
            <p onClick={handleAddTeacher} className="items-center p-3 ">
              Add Teacher
            </p>
          </div>
        </div>
              ...
              {taskToDisplay?.map((item) => (
                ...
                >
                  <div
                    ...
                    onClick={(event) => handleClick(item?._id, event)}
                  >
                    <AiFillWechat />
                  </div>
